...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 5, "cvodeieq");
 	...
 	ivoc_help("help ?1 ch_CavL C:/Users/DELL 111/Documents/Boglárka nagyon komoly mappája/Suli muri/TDK szakdoga stuff/Kosársejt modellek/Modell csupaszítás/CA1 modellek/CA1 Lee et al. 2014/ch_CavL.mod\n");
...
double ghk (  double _lv , double _lci , double _lco ) {
 ...
 _lf = KTF ( _threadargscomma_ celsius ) / 2.0 ;
   _lghk = - _lf * ( 1. - ( _lci / _lco ) * exp ( _lv / _lf ) ) * vtrap ( _threadargscomma_ _lv , _lf ) / _lf ;
...
double alp (  double _lv ) {
   double _lalp;
 _lalp = 15.69 * vtrap ( _threadargscomma_ ( - 1.0 * _lv + 81.5 ) , 10.0 ) ;
 ...
 v=_v;
{
  ...
  eca = _ion_eca;
 { error =  state();
 if(error){fprintf(stderr,"at line 65 in file ch_CavL.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
