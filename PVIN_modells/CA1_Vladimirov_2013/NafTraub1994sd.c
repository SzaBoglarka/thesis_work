...
#define nrn_init _nrn_init__NaFsd
#define _nrn_initial _nrn_initial__NaFsd
#define nrn_cur _nrn_cur__NaFsd
#define _nrn_current _nrn_current__NaFsd
#define nrn_jacob _nrn_jacob__NaFsd
#define nrn_state _nrn_state__NaFsd
#define _net_receive _net_receive__NaFsd 
#define _f_rates _f_rates__NaFsd 
#define rates rates__NaFsd 
#define states states__NaFsd 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_NaFsd", _hoc_setdata,
 "rates_NaFsd", _hoc_rates,
 0, 0
};
...
#define Vm Vm_NaFsd
 double Vm = -65;
#define usetable usetable_NaFsd
 double usetable = 1;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 NaFsd C:/Users/DELL 111/Documents/Boglárka nagyon komoly mappája/Suli muri/TDK szakdoga stuff/Kosársejt modellek/Modell csupaszítás/CA1 modellek/CA1 Vladimirov 2013/NafTraub1994sd.mod\n");
...
static int _reset;
static char *modelname = "Fast sodium current, for soma-dendrites compartments";
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
   double _lalpham , _lbetam , _lalphah , _lbetah , _lsmall ;
  _lsmall = ( 13.1 - ( _lv - Vm ) ) / 4.0 ;
   if ( fabs ( _lsmall ) > 1e-6 ) {
     _lalpham = 0.32 * ( 13.1 - ( _lv - Vm ) ) / ( exp ( ( 13.1 - ( _lv - Vm ) ) / 4.0 ) - 1.0 ) ;
     }
   else {
     _lalpham = 0.32 * 4.0 / ( 1.0 + _lsmall / 2.0 ) ;
     }
   _lsmall = ( ( _lv - Vm ) - 40.1 ) / 5.0 ;
   if ( fabs ( _lsmall ) > 1e-6 ) {
     _lbetam = 0.28 * ( ( _lv - Vm ) - 40.1 ) / ( exp ( ( ( _lv - Vm ) - 40.1 ) / 5.0 ) - 1.0 ) ;
     }
   else {
     _lbetam = 0.28 * 5.0 / ( 1.0 + _lsmall / 2.0 ) ;
     }
   minf = _lalpham / ( _lalpham + _lbetam ) ;
   mtau = 1.0 / ( _lalpham + _lbetam ) ;
   _lalphah = 0.128 * exp ( ( 17.0 - ( _lv - Vm ) ) / 18.0 ) ;
   _lbetah = 4.0 / ( 1.0 + exp ( ( 40.0 - ( _lv - Vm ) ) / 5.0 ) ) ;
   hinf = _lalphah / ( _lalphah + _lbetah ) ;
   htau = 1.0 / ( _lalphah + _lbetah ) ;
     return 0; }
...
#if NMODL_TEXT
static const char* nmodl_filename = "NafTraub1994sd.mod";
static const char* nmodl_file_text = 
