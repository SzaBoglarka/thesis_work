...
#define _f_rates _f_rates__Kdrax 
#define rates rates__Kdrax 
...
#define Vm Vm_Kdrax
 double Vm = -65;
#define usetable usetable_Kdrax
 double usetable = 1;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 Kdrax C:/Users/DELL 111/Documents/Boglárka nagyon komoly mappája/Suli muri/TDK szakdoga stuff/Kosársejt modellek/Modell csupaszítás/CA1 modellek/CA1 Vladimirov 2013/KdrTraub1994ax.mod\n");
...
static int _reset;
static char *modelname = "Delayed rectifier K current, for axonal compartments";
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
  ...
  _lsmall = ( 17.2 - ( _lv - Vm ) ) / 5.0 ;
   if ( fabs ( _lsmall ) > 1e-6 ) {
     _lalphan = 0.03 * ( 17.2 - ( _lv - Vm ) ) / ( exp ( ( 17.2 - ( _lv - Vm ) ) / 5.0 ) - 1.0 ) ;
     }
   else {
     _lalphan = 0.03 * 5.0 / ( 1.0 + _lsmall / 2.0 ) ;
     }
   _lbetan = 0.45 * exp ( ( 12.0 - ( _lv - Vm ) ) / 40.0 ) ;
...
#if NMODL_TEXT
static const char* nmodl_filename = "KdrTraub1994ax.mod";
static const char* nmodl_file_text = 
