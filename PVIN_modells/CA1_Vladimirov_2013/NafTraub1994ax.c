...
#define nrn_init _nrn_init__NaFax
#define _nrn_initial _nrn_initial__NaFax
#define nrn_cur _nrn_cur__NaFax
#define _nrn_current _nrn_current__NaFax
#define nrn_jacob _nrn_jacob__NaFax
#define nrn_state _nrn_state__NaFax
#define _net_receive _net_receive__NaFax 
#define _f_rates _f_rates__NaFax 
#define rates rates__NaFax 
#define states states__NaFax 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_NaFax", _hoc_setdata,
 "rates_NaFax", _hoc_rates,
 0, 0
};
...
#define Vm Vm_NaFax
 double Vm = -65;
#define usetable usetable_NaFax
 double usetable = 1;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 NaFax C:/Users/DELL 111/Documents/Boglárka nagyon komoly mappája/Suli muri/TDK szakdoga stuff/Kosársejt modellek/Modell csupaszítás/CA1 modellek/CA1 Vladimirov 2013/NafTraub1994ax.mod\n");
...
static int _reset;
static char *modelname = "Fast sodium current, for axonal compartments";
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
  ...
  _lsmall = ( 17.2 - ( _lv - Vm ) ) / 4.0 ;
   if ( fabs ( _lsmall ) > 1e-6 ) {
     _lalpham = 0.8 * ( 17.2 - ( _lv - Vm ) ) / ( exp ( ( 17.2 - ( _lv - Vm ) ) / 4.0 ) - 1.0 ) ;
     }
   else {
     _lalpham = 0.8 * 4.0 / ( 1.0 + _lsmall / 2.0 ) ;
     }
   _lsmall = ( ( _lv - Vm ) - 42.2 ) / 5.0 ;
   if ( fabs ( _lsmall ) > 1e-6 ) {
     _lbetam = 0.7 * ( ( _lv - Vm ) - 42.2 ) / ( exp ( ( ( _lv - Vm ) - 42.2 ) / 5.0 ) - 1.0 ) ;
     }
   else {
     _lbetam = 0.7 * 5.0 / ( 1.0 + _lsmall / 2.0 ) ;
     }
   minf = _lalpham / ( _lalpham + _lbetam ) ;
   mtau = 1.0 / ( _lalpham + _lbetam ) ;
   _lalphah = 0.32 * exp ( ( 42.0 - ( _lv - Vm ) ) / 18.0 ) ;
   _lbetah = 10.0 / ( 1.0 + exp ( ( 42.0 - ( _lv - Vm ) ) / 5.0 ) ) ;
...
#if NMODL_TEXT
static const char* nmodl_filename = "NafTraub1994ax.mod";
static const char* nmodl_file_text = 
