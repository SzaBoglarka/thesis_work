...
def plot_isi_data_with_log_fit(cells_data, species, ax, x_length=None, color='blue',
    ...
    for i, isi_values in enumerate(isi_data_list):
        ...
        # Apply moving average on the ISI_values
        window_size = 6  # window size for the moving average
        # Compute the moving average using np.convolve
        moving_avg = np.convolve(isi_values, np.ones(window_size) / window_size, mode='valid')
        # Adjust x_values to match the moving average output length
        x_values_moving_avg = x_values[(window_size - 1) // 2:-(window_size - 1) // 2 or None]
        ax.plot(x_values_moving_avg, moving_avg, color=colors_ordered[i], linestyle='--', alpha=0.7,
                label=f'Moving Avg (window={window_size})')  # Plot the moving average line
...
plot_all = False
save_fig = f'./figures/MOUSE_vs_RAT_ISI_values_{current_type}_until_800ms_colorful_from_second_moving_average.png'
...
ax_mouse.set_title(f"{currents[current_type]['name']} Current ISI Values: MOUSE")
ax_rat.set_title(f"{currents[current_type]['name']} Current ISI Values: RAT")
# Set ylim
if plot_all:
