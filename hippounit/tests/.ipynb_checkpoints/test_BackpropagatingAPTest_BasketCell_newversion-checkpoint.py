...
class test_BackpropagatingAPTest_BasketCell_newversion(Test):
    ...
    def extract_somatic_spiking_features(self, traces_soma_and_apical, traces_soma_and_basal, delay, duration):
        ...
        #efel.set_double_setting('interp_step', 0.025)
        #efel.set_double_setting('DerivativeThreshold', 40.0)

        efel_features_somatic = efel.get_feature_values(traces_soma, ['inv_first_ISI','AP_begin_time', 'doublet_ISI', 'peak_indices', 'ISI_values'])
        ...
        return efel_features_somatic


    def get_time_indices_and_extract_amplitudes(self, efel_features_somatic, traces_soma_and_apical,
        traces_soma_and_basal, apical_locations_distances, basal_locations_distances):
        ...
        soma_AP_peaks = efel_features_somatic[0]['peak_indices']
        print("soma_AP_peaks:", soma_AP_peaks)
        soma_AP_begin_time = efel_features_somatic[0]['AP_begin_time']
        soma_first_ISI = efel_features_somatic[0]['ISI_values'][0]
        print("soma_first_ISI:", soma_first_ISI)
        soma_second_ISI = efel_features_somatic[0]['ISI_values'][1]
        print("soma_second_ISI:", soma_second_ISI)

        print("type(soma_AP_begin_time[0])", type(soma_AP_begin_time[0]))
        print("type(soma_AP_peaks[0])", type(soma_AP_peaks[0]))
        s_indices_AP1 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_peaks[0]-2.0))


        print("-1")
        s_indices_AP1 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_peaks[0]-2.0))
        print("soma_AP_peaks[0]-2.0:", soma_AP_peaks[0]-2.0)
        print("s_indices_AP1:", s_indices_AP1)
        # we might use the second AP instead - the detection of the 1st AP is not robust !
        s_indices_AP2 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[1] - 2.0))
        print("soma_AP_peaks[1]-2.0:", soma_AP_peaks[1] - 2.0)
        print("s_indices_AP2:", s_indices_AP2)
        plus = soma_second_ISI-1.5
        e_indices_AP1 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_peaks[0]+plus))
        e_indices_AP2 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[1] + plus))
        print("0")

        #print("s_indices_AP1[0]", s_indices_AP1[0])
        #start_index_AP1 = s_indices_AP1[0][0]
        #print("start_index_AP1:", start_index_AP1)
        #end_index_AP1 = e_indices_AP1[0][0]
        start_index_AP2 = s_indices_AP2[0][0]
        print("start_index_AP2:", start_index_AP2)
        end_index_AP2 = e_indices_AP2[0][0]
        print("end_index_AP2:", end_index_AP2)

        # For now as a test:
        start_index_AP1 = start_index_AP2
        end_index_AP1 = end_index_AP2

        print("1")

        #s_indices_APlast = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_peaks[-1]-2.0))
        #e_indices_APlast = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_peaks[-1]+plus))
        s_indices_APlast = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[-1] - 2.0))
        e_indices_APlast = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[-1] + plus))
        start_index_APlast = s_indices_APlast[0][0]
        end_index_APlast = e_indices_APlast[0][0]

        print("2")

        somatic_features = collections.OrderedDict()
        AP_amplitudes = []

        for i in range(1,len(soma_AP_begin_time)):
            s_indices_AP = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[1] - 2.0))
            e_indices_AP = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[1] + plus))
            start_index_AP = s_indices_AP[0][0]
            end_index_AP = e_indices_AP[0][0]
            AP_amp= float(numpy.amax(traces_soma_and_apical['v_stim'][start_index_AP:end_index_AP]) -
                           traces_soma_and_apical['v_stim'][start_index_AP])*mV
            AP_amplitudes.append(AP_amp)
            #somatic_features['AP_amplitudes'].append(AP_amp)

        somatic_features['AP_amplitude'] = {}
        somatic_features['AP_amplitude']['mean'] = numpy.mean(AP_amplitudes)
        somatic_features['AP_amplitude']['std'] = numpy.std(AP_amplitudes)
        ...
        '''
        if AP1_amp < 0*mV:
            s_indices_AP1 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[1]-soma_first_ISI-1.0))
            e_indices_AP1 = numpy.where(traces_soma_and_apical['T'] >= (soma_AP_begin_time[1]-1.5))
            start_index_AP1 = s_indices_AP1[0][0]
            end_index_AP1 = e_indices_AP1[0][0]
            AP1_amp= float(numpy.amax(traces_soma_and_apical['v_stim'][start_index_AP1:end_index_AP1]) - traces_soma_and_apical['v_stim'][start_index_AP1])*mV
        '''


        '''
        somatic_features['AP_amplitudes'].append(AP1_amp)
        for i in range(len(soma_AP_begin_time)):
            AP_amp = 
        '''

        somatic_features['AP1_amp'] = (
                float(numpy.amax(traces_soma_and_apical['v_stim'][start_index_AP1:end_index_AP1])
                      - traces_soma_and_apical['v_stim'][start_index_AP1]) * mV)

        somatic_features['APlast_amp'] = (
                float(numpy.amax(traces_soma_and_apical['v_stim'][start_index_APlast:end_index_APlast])
                      - traces_soma_and_apical['v_stim'][start_index_APlast]) *mV)

        print("somatic_features:",somatic_features)

        apical_features = collections.OrderedDict()

        for key, value in traces_soma_and_apical['v_rec'].items():
            apical_features[key] = collections.OrderedDict()

            apical_features[key]['AP1_amp'] = (
                    float(numpy.amax(traces_soma_and_apical['v_rec'][key][start_index_AP1:end_index_AP1])
                          - traces_soma_and_apical['v_rec'][key][start_index_AP1]) *mV)
            apical_features[key]['APlast_amp'] = (
                    float(numpy.amax(traces_soma_and_apical['v_rec'][key][start_index_APlast:end_index_APlast]) -
                          traces_soma_and_apical['v_rec'][key][start_index_APlast])*mV)
            #apical_features[key]['apical_locations_distances'] = apical_locations_distances[key]

        print("apical_features:",apical_features)

        basal_features = collections.OrderedDict()

        for key, value in traces_soma_and_basal['v_rec'].items():
            basal_features[key] = collections.OrderedDict()

            basal_features[key]['AP1_amp'] = (
                    float(numpy.amax(traces_soma_and_basal['v_rec'][key][start_index_AP1:end_index_AP1])
                          - traces_soma_and_basal['v_rec'][key][start_index_AP1])*mV)
            basal_features[key]['APlast_amp'] = (
                    float(numpy.amax(traces_soma_and_basal['v_rec'][key][start_index_APlast:end_index_APlast])
                          - traces_soma_and_basal['v_rec'][key][start_index_APlast])*mV)
            #basal_features[key]['basal_locations_distances'] = basal_locations_distances[key]

        print("basal_features:",basal_features)

        return [start_index_AP1, end_index_AP1, start_index_APlast, end_index_APlast], somatic_features, apical_features, basal_features
    ...
    def extract_prediction_features(self, features_somatic, features_apical, features_basal, apical_locations_distances, basal_locations_distances, distances_apical, tolerance_apical, distances_basal, tolerance_basal):

        #print("features_somatic['AP1_amp']:", features_somatic['AP1_amp'])

        features = {'soma' : { 'AP1_amplitude' : features_somatic['AP1_amp']*mV, 
                               'APlast_amplitude' : features_somatic['APlast_amp']*mV},
                               'AP_amplitude' :
                                {'mean': features_somatic['AP_amplitude']['mean'],
                                 'std': features_somatic['AP_amplitude']['std']},
...
#                              'AP_rise_slope' : (numpy.mean(efel_features_somatic[0]['AP_rise_rate'])/1000.0)*V/ms},  # converting from V/s to V/ms 
        ...
        print("We made in even here wuuuut")

        features_to_json = {'soma' : { 'AP1_amplitude' : str(features_somatic['AP1_amp']*mV),
                                       'APlast_amplitude' : str(features_somatic['APlast_amp']*mV)},
                                       'AP_amplitude' :
                                           {'mean': features_somatic['AP_amplitude']['mean'],
                                            'std': features_somatic['AP_amplitude']['std']},
...
#                                              'APlast_rise_slope' : str((value[0]['AP_rise_rate'][-1]/1000.0)*V/ms)
        ...
        print("hahahaha, megvagyunk idáig sirály")
        ...
        prediction_to_json = {'soma' : { 'AP1_amplitude' : str(features['soma']['AP1_amplitude']),
                                         'APlast_amplitude' : str(features['soma']['APlast_amplitude'])},
...
#            axs2[2, 1].errorbar(int(key)*-1.0, value['APlast_half_duration']['mean'], yerr = value['APlast_half_duration']['std'], marker='o', linestyle='none', color='blue')   
    ...
    def generate_prediction(self, model, verbose=False):
        ...
        # print("dend_locations:", dend_locations)
        ...
        #print("traces['v_rec'].items():", traces['v_rec'].items())
        ...
        #print("traces_soma_and_apical:",traces_soma_and_apical)
        #print("traces_soma_and_basal",traces_soma_and_basal)       
        ...
        efel_features_somatic = self.extract_somatic_spiking_features(traces_soma_and_apical, traces_soma_and_basal, delay, duration)

        #print("features_somatic:",efel_features_somatic)

        #old version: time_indices_befor_and_after_somatic_AP = self.get_time_indices_befor_and_after_somatic_AP(efel_features_somatic, traces_soma_and_apical)
        time_indices_befor_and_after_somatic_AP, features_somatic, features_apical, features_basal = self.get_time_indices_and_extract_amplitudes(efel_features_somatic, traces_soma_and_apical, traces_soma_and_basal, apical_locations_distances, basal_locations_distances)

        print("The somatic, apical and basal AP1 and APlast amplitudes are extracted")
        ...
        features, prediction = self.extract_prediction_features(features_somatic, features_apical, features_basal, apical_locations_distances, basal_locations_distances, distances_apical, tolerance_apical, distances_basal, tolerance_basal)

        print("The features and the prediction is created.")
